...
void createServer(int family, int port_no, coap_resource_t *) {
  coap_context_t *ctx;
  coap_address_t serv_addr;
  fd_set readfds;
  coap_address_init(&serv_addr);
  serv_addr.addr.sin.sin_family = AF_INET; // family
  serv_addr.addr.sin.sin_addr.s_addr = INADDR_ANY;
  serv_addr.addr.sin.sin_port = htons(5683); // default port//port_no
  ctx = coap_new_context(&serv_addr);
  if (!ctx)
    exit(EXIT_FAILURE);
}
