...
void IProute::attributes_show_ipv4(struct nlattr *tb[])
{
	if (tb[RTA_TABLE]) {
...
void IProute::attributes_show_ipv6(struct nlattr *tb[])
{
	if (tb[RTA_TABLE]) {
		cout<<"Table= "<< mnl_attr_get_u32(tb[RTA_TABLE])<<endl;
	}
...
int IProute::data_cb_showIproutes(const struct nlmsghdr *nlh, void *data){
  ...
  cout<<"Family="<<((rm->rtm_family == AF_INET) ? "IPv4" : "IPv6");// protocol family = AF_INET | AF_INET6
  cout<<"\tDst_len="<< (int)rm->rtm_dst_len;//destination CIDR, eg. 24 or 32 for IPv4
